...
/// <summary>
/// OData controller for retrieving master project units.
/// </summary>
[Authorize]
public class ProjectUnitController(
    ILogger<ProjectUnitController> logger,
    IProjectUnitBusiness projectUnitBusiness
) : ODataController
{
    private const string ClassName = nameof(ProjectUnitController);
    ...
    public async Task<IActionResult> GetAsync()
    {
        ...
        try
        {
            ...
            var result = await projectUnitBusiness.GetAsync();
            return Ok(result);
        }
        catch (Exception ex)
        {
    ...
    /// <summary>
    /// Retrieves a project unit by its row identifier (GUID).
    /// </summary>
    /// <param name="rowId">The unique row identifier (GUID) of the project unit.</param>
    /// <returns>200 OK with the project unit; 404 if not found; 500 on error.</returns>
    [HttpGet("v1/ProjectUnit({rowId:guid})")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    ...
    public async Task<IActionResult> GetByRowIdAsync([FromRoute] Guid rowId)
    {
        ...
        try
        {
            ...
            var query = await projectUnitBusiness.GetAsync();
            var item = query.FirstOrDefault(x => x.RowId == rowId);
            if (item == null)
            {
                logger.LogWarning("{MethodName} - Project unit with id {Id} not found", methodName, rowId);
                return NotFound($"Project unit with id {rowId} not found");
            }
            return Ok(item);
        }
        catch (Exception ex)
