...
/// <summary>
/// Integration-style tests for <see cref="KonaAI.Master.API.Controllers.Master.MetaData.ModuleController"/>.
/// Covers:
...
{
    // Overview:
    // - Exercises ModuleController actions with mocked business dependencies.
    ...
    private static ModuleController CreateController(
        out Mock<IModuleTypeBusiness> business)
    {
        var logger = new Mock<ILogger<ModuleController>>();
        business = new Mock<IModuleTypeBusiness>(MockBehavior.Strict);
        return new ModuleController(logger.Object, business.Object);
    }
    ...
    public async Task GetByRowIdAsync_ReturnsOk_WhenFound()
    {
        ...
        var data = new List<MetaDataViewModel>
        {
            new() { RowId = rowId, Name = "Module 1", Description = "First module" }
        }.AsQueryable();
        business.Setup(b => b.GetAsync()).ReturnsAsync(data);
