...
/// <summary>
/// Integration-style tests for <see cref="KonaAI.Master.API.Controllers.Master.MetaData.NavigationActionController"/>.
/// Covers:
/// - GET list (200) for read-only metadata controller
/// - GET by RowId (200 or 404)
/// - Error handling (500) for server errors
/// </summary>
public class NavigationActionControllerTests
{
    // Overview:
    // - Exercises NavigationActionController actions with mocked business dependencies.
    ...
    private static NavigationActionController CreateController(
        out Mock<INavigationUserActionBusiness> business)
    {
        var logger = new Mock<ILogger<NavigationActionController>>();
        business = new Mock<INavigationUserActionBusiness>(MockBehavior.Strict);
        return new NavigationActionController(logger.Object, business.Object);
    }
    ...
    public async Task GetAsync_ReturnsOk_WithQueryable()
    {
        ...
        {
            new() { RowId = Guid.NewGuid(), NavigationName = "Navigation 1", UserActionName = "Action 1" }
        }.AsQueryable();
    ...
    public async Task GetByRowIdAsync_ReturnsOk_WhenFound()
    {
        ...
        var data = new List<NavigationUserActionViewModel>
        {
            new() { RowId = rowId, NavigationName = "Navigation 1", UserActionName = "Action 1" }
        }.AsQueryable();
        business.Setup(b => b.GetAsync()).ReturnsAsync(data);
    ...
    public async Task GetByRowIdAsync_ReturnsNotFound_WhenMissing()
    {
        ...
        var controller = CreateController(out var business);
        var rowId = Guid.NewGuid();
        var data = new List<NavigationUserActionViewModel>().AsQueryable();
        business.Setup(b => b.GetAsync()).ReturnsAsync(data);
