...
/// <summary>
/// Integration-style tests for <see cref="KonaAI.Master.API.Controllers.Master.MetaData.ProjectAuditResponsibilityController"/>.
/// Covers:
/// - GET list (200) for read-only metadata controller
/// - GET by RowId (200 or 404)
/// - Error handling (500) for server errors
/// </summary>
public class ProjectAuditResponsibilityControllerTests
{
    // Overview:
    // - Exercises ProjectAuditResponsibilityController actions with mocked business dependencies.
    ...
    private static ProjectAuditResponsibilityController CreateController(
        out Mock<IProjectAuditResponsibilityBusiness> business)
    {
        var logger = new Mock<ILogger<ProjectAuditResponsibilityController>>();
        business = new Mock<IProjectAuditResponsibilityBusiness>(MockBehavior.Strict);
        return new ProjectAuditResponsibilityController(logger.Object, business.Object);
    }
    ...
    public async Task GetByRowIdAsync_ReturnsOk_WhenFound()
    {
        ...
        var data = new List<MetaDataViewModel>
        {
            new() { RowId = rowId, Name = "Audit Responsibility 1", Description = "First audit responsibility" }
        }.AsQueryable();
        business.Setup(b => b.GetAsync()).ReturnsAsync(data);
