...
/// <summary>
/// Integration-style tests for <see cref="KonaAI.Master.API.Controllers.Master.MetaData.ProjectDepartmentController"/>.
/// Covers:
/// - GET list (200) for read-only metadata controller
/// - GET by RowId (200 or 404)
/// - Error handling (500) for server errors
/// </summary>
public class ProjectDepartmentControllerTests
{
    // Overview:
    // - Exercises ProjectDepartmentController actions with mocked business dependencies.
    ...
    private static ProjectDepartmentController CreateController(
        out Mock<IProjectDepartmentBusiness> business)
    {
        var logger = new Mock<ILogger<ProjectDepartmentController>>();
        business = new Mock<IProjectDepartmentBusiness>(MockBehavior.Strict);
        return new ProjectDepartmentController(logger.Object, business.Object);
    }
    ...
    public async Task GetByRowIdAsync_ReturnsOk_WhenFound()
    {
        ...
        var data = new List<MetaDataViewModel>
        {
            new() { RowId = rowId, Name = "Department 1", Description = "First department" }
        }.AsQueryable();
        business.Setup(b => b.GetAsync()).ReturnsAsync(data);
