...
/// <summary>
/// Integration-style tests for <see cref="KonaAI.Master.API.Controllers.Tenant.UserMetaData.ClientProjectCountryController"/>.
/// Covers:
/// - GET list (200) for read-only metadata controller
/// - GET by RowId (200 or 404)
/// - Error handling (500) for server errors
/// </summary>
public class ClientProjectCountryControllerTests
{
    // Overview:
    // - Exercises ClientProjectCountryController actions with mocked business dependencies.
    ...
    private static ClientProjectCountryController CreateController(
        out Mock<IClientProjectCountryBusiness> business)
    {
        var logger = new Mock<ILogger<ClientProjectCountryController>>();
        business = new Mock<IClientProjectCountryBusiness>(MockBehavior.Strict);
        return new ClientProjectCountryController(logger.Object, business.Object);
    }
    ...
    public async Task GetAsync_ReturnsOk_WithQueryable()
    {
        ...
        {
            new() { RowId = Guid.NewGuid(), Name = "Country 1", Description = "First country" }
        }.AsQueryable();
