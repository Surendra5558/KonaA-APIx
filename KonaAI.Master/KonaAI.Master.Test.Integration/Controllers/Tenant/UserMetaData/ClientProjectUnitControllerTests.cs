...
/// <summary>
/// Integration-style tests for <see cref="KonaAI.Master.API.Controllers.Tenant.UserMetaData.ClientProjectUnitController"/>.
/// Covers:
/// - GET list (200) for read-only metadata controller
/// - GET by RowId (200 or 404)
/// - Error handling (500) for server errors
/// </summary>
public class ClientProjectUnitControllerTests
{
    // Overview:
    // - Exercises ClientProjectUnitController actions with mocked business dependencies.
    ...
    private static ClientProjectUnitController CreateController(
        out Mock<IClientProjectUnitBusiness> business)
    {
        var logger = new Mock<ILogger<ClientProjectUnitController>>();
        business = new Mock<IClientProjectUnitBusiness>(MockBehavior.Strict);
        return new ClientProjectUnitController(logger.Object, business.Object);
    }
