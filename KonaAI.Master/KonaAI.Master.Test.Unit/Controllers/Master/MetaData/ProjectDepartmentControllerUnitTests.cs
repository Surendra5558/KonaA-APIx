...
public class ProjectDepartmentControllerUnitTests
{
    private readonly Mock<ILogger<ProjectDepartmentController>> _logger = new();
    private readonly Mock<IProjectDepartmentBusiness> _business = new();

    private ProjectDepartmentController CreateSut() =>
    ...
    public async Task GetAsync_Returns200_WithData()
    {
        var data = new List<MetaDataViewModel>
        {
            ...
            new() { RowId = Guid.NewGuid(), Name = "HR", Description = "Human Resources", OrderBy = 2 }
        }.AsQueryable();
    ...
    public async Task GetByRowIdAsync_Found_Returns200()
    {
        ...
        var data = new List<MetaDataViewModel>
        {
            new() { RowId = id, Name = "IT", Description = "Information Technology", OrderBy = 1 }
        }.AsQueryable();
    ...
    {
        var attr = typeof(ProjectDepartmentController).GetCustomAttribute<AuthorizeAttribute>(inherit: true);
        Assert.NotNull(attr);
    }
    ...
    {
        var mi = typeof(ProjectDepartmentController).GetMethod(nameof(ProjectDepartmentController.GetAsync));
        Assert.NotNull(mi);
        var classAttr = typeof(ProjectDepartmentController).GetCustomAttribute<AuthorizeAttribute>(inherit: true);
