...
public class ProjectRiskAreaControllerUnitTests
{
    private readonly Mock<ILogger<ProjectRiskAreaController>> _logger = new();
    private readonly Mock<IProjectRiskAreaBusiness> _business = new();

    private ProjectRiskAreaController CreateSut() =>
    ...
    public async Task GetAsync_Returns200_WithData()
    {
        ...
        {
            new() { RowId = Guid.NewGuid(), Name = "High", Description = "High Risk", OrderBy = 1 },
            new() { RowId = Guid.NewGuid(), Name = "Medium", Description = "Medium Risk", OrderBy = 2 }
        }.AsQueryable();
    ...
    public async Task GetByRowIdAsync_Found_Returns200()
    {
        ...
        var data = new List<MetaDataViewModel>
        {
            new() { RowId = id, Name = "High", Description = "High Risk", OrderBy = 1 }
        }.AsQueryable();
    ...
    {
        var attr = typeof(ProjectRiskAreaController).GetCustomAttribute<AuthorizeAttribute>(inherit: true);
        Assert.NotNull(attr);
    }
    ...
    {
        var mi = typeof(ProjectRiskAreaController).GetMethod(nameof(ProjectRiskAreaController.GetAsync));
        Assert.NotNull(mi);
        var classAttr = typeof(ProjectRiskAreaController).GetCustomAttribute<AuthorizeAttribute>(inherit: true);
