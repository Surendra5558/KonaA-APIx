...
public class ClientProjectAuditResponsibilityControllerUnitTests
{
    private readonly Mock<ILogger<ClientProjectAuditResponsibilityController>> _logger = new();
    private readonly Mock<IClientProjectAuditResponsibilityBusiness> _business = new();

    private ClientProjectAuditResponsibilityController CreateSut() =>
    ...
    public async Task GetAsync_Returns200_WithData()
    {
        ...
        {
            new() { RowId = Guid.NewGuid(), Name = "Primary", Description = "Primary Responsibility", OrderBy = 1 }
        }.AsQueryable();
    ...
    {
        var attr = typeof(ClientProjectAuditResponsibilityController).GetCustomAttribute<AuthorizeAttribute>(inherit: true);
        Assert.NotNull(attr);
    }
    ...
    {
        var mi = typeof(ClientProjectAuditResponsibilityController).GetMethod(nameof(ClientProjectAuditResponsibilityController.GetAsync));
        Assert.NotNull(mi);
        var classAttr = typeof(ClientProjectAuditResponsibilityController).GetCustomAttribute<AuthorizeAttribute>(inherit: true);
