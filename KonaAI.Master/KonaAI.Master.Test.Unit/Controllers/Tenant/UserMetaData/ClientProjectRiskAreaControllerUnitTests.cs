...
public class ClientProjectRiskAreaControllerUnitTests
{
    private readonly Mock<ILogger<ClientProjectRiskAreaController>> _logger = new();
    private readonly Mock<IClientProjectRiskAreaBusiness> _business = new();

    private ClientProjectRiskAreaController CreateSut() =>
    ...
    {
        var attr = typeof(ClientProjectRiskAreaController).GetCustomAttribute<AuthorizeAttribute>(inherit: true);
        Assert.NotNull(attr);
    }
    ...
    {
        var mi = typeof(ClientProjectRiskAreaController).GetMethod(nameof(ClientProjectRiskAreaController.GetAsync));
        Assert.NotNull(mi);
        var classAttr = typeof(ClientProjectRiskAreaController).GetCustomAttribute<AuthorizeAttribute>(inherit: true);
