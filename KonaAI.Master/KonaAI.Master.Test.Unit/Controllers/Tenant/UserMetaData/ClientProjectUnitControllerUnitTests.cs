...
public class ClientProjectUnitControllerUnitTests
{
    private readonly Mock<ILogger<ClientProjectUnitController>> _logger = new();
    private readonly Mock<IClientProjectUnitBusiness> _business = new();

    private ClientProjectUnitController CreateSut() =>
    ...
    {
        var attr = typeof(ClientProjectUnitController).GetCustomAttribute<AuthorizeAttribute>(inherit: true);
        Assert.NotNull(attr);
    }
    ...
    {
        var mi = typeof(ClientProjectUnitController).GetMethod(nameof(ClientProjectUnitController.GetAsync));
        Assert.NotNull(mi);
        var classAttr = typeof(ClientProjectUnitController).GetCustomAttribute<AuthorizeAttribute>(inherit: true);
